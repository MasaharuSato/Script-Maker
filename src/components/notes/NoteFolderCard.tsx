...
export function NoteFolderCard({ id, name, noteCount, onDelete }: NoteFolderCardProps) {
  return (
    <div
      className="group relative rounded-lg bg-bg-secondary p-4 transition-all hover:bg-bg-tertiary active:scale-[0.98]"
      style={{ boxShadow: 'var(--shadow-card)' }}
    >
      <Link href={`/notes/folder/${id}`} className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-text-muted mt-0.5">{noteCount}件のメモ</p>
        </div>
      </Link>
      <button
        onClick={(e) => {
